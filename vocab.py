...
def main():
    # 直接在代码里写参数
    args = {
        'TRAIN': './data/NER-train-utf8.txt',  # 训练数据文件路径
        'SENT_VOCAB': './vocab/sent_vocab.json',  # 句子词汇表输出路径
        'TAG_VOCAB': './vocab/tag_vocab.json',  # 标签词汇表输出路径
        '--max-size': '20000',  # 词汇表最大大小
        '--freq-cutoff': '2'  # 频率阈值
    }
    ...
    print(f"句子词汇表已保存到: {args['SENT_VOCAB']}, 大小: {len(sent_vocab)}")
    print(f"标签词汇表已保存到: {args['TAG_VOCAB']}, 大小: {len(tag_vocab)}")
