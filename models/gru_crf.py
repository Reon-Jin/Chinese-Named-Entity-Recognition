...
class GRUCRF(nn.Module):
    def __init__(self, sent_vocab, tag_vocab, dropout_rate=0.5, embed_size=256, hidden_size=256):
        ...
        self.encoder = nn.GRU(input_size=embed_size, hidden_size=hidden_size, bidirectional=True)
    ...
    def load(filepath, device_to_load):
        ...
        model = GRUCRF(params['sent_vocab'], params['tag_vocab'], **params['args'])
...
def main():
    ...
    model = GRUCRF(sent_vocab, tag_vocab)
    model.to(device)
    model.save('../trained_model/GRU/model.pth')
    model = model.load('../model/GRU/model.pth', device)
